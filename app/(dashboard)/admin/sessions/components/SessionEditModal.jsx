...
export default function SessionEditModal({
  ...
  const formatDateForInput = (dateStr, timeStr) => {
    try {
      ...
      // If we already received ISO strings, pass through
      ...
      // Adjust to local for datetime-local input (no timezone)
  ...
  useEffect(() => {
    if (session) {
      // session.date is yyyy-mm-dd, session.startTime like 9:30:00 AM or 09:30:00
      const normTime = (t) => {
        if (!t) return "";
        // If already includes AM/PM, construct from locale
        const parsed = Date.parse(`${session.date} ${t}`);
        if (!isNaN(parsed)) {
        ...
        // Fallback: return first 5 chars of HH:mm:ss
        return t.slice(0,5);
      };

      const startDateTime = formatDateForInput(session.date, normTime(session.startTime));
      const endDateTime = formatDateForInput(session.date, normTime(session.endTime));
      
      setFormData({
        startTime: startDateTime,
        endTime: endDateTime,
        status: session.status || "pending",
        sessionType: session.sessionType || session.location || "virtual",
        notes: session.notes || "",
  ...
  // Handle input changes (no auto-update; only update on Save)
