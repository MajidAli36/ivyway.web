...
const SessionCard = ({ session, onView, onUpdateStatus, userRole }) => {
  ...
  const getOtherUserInfo = (session, userRole) => {
    if (userRole === 'counselor') {
      ...
      return {
        name: session.counselor?.name || session.counselorName || 'Unknown Counselor',
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex space-x-2">
              ...
              {onUpdateStatus && (session.status === 'accepted' || session.status === 'confirmed') && (
                <button
                  onClick={() => onUpdateStatus(session)}
...
const SessionModal = ({ isOpen, closeModal, session, onUpdateStatus, userRole }) => {
              ...
              <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                ...
                <div className="mt-4 space-y-3">
                  ...
                  <div className="text-sm text-gray-600">
                    ...
                    <p><strong>Status:</strong> <StatusBadge status={session.status} /></p>
                  ...
                  {onUpdateStatus && (session.status === 'accepted' || session.status === 'confirmed') && (
                    <div className="mt-4">
                      ...
                      >
                        ...
                        <option value="confirmed">Confirm Session</option>
...
export default function StudentSessionsPage() {
  ...
  const [userRole, setUserRole] = useState('student'); // Will be determined from auth
  ...
  useEffect(() => {
    const checkAuth = () => {
      ...
      // For now, we'll assume student based on the route
      setUserRole('student');
  ...
  useEffect(() => {
    const fetchSessions = async () => {
      ...
      if (result.success) {
        ...
        const uniqueSessions = ensureArray(result.data).filter((session, index, self) => 
        ...
        const uniqueMockSessions = mockSessions.filter((session, index, self) => 
          index === self.findIndex(s => s.id === session.id)
        );
  ...
  const handleUpdateStatus = async (session, newStatus) => {
    try {
      ...
      const result = await safeApiCall(
        () => counselorBookings.updateSessionStatus(session.id, { status: newStatus }),
        {
          extractArray: false,
          dataKey: 'data',
          defaultData: null,
      ...
      if (result.success) {
        ...
        console.log("Session status updated successfully:", session.id, "New status:", newStatus);
        setNotification({ type: 'success', message: 'Session status updated successfully!' });
      } else {
        ...
        if (result.error.includes("not found") || result.error.includes("404")) {
          console.log("API endpoint not implemented, simulating status update...");
          ...
          setNotification({ 
            type: 'success', 
            message: 'Session status updated! (Note: Backend endpoint not yet implemented)' 
          });
        } else {
          setNotification({ type: 'error', message: `Failed to update session status: ${result.error}` });
        }
      }
    } catch (error) {
