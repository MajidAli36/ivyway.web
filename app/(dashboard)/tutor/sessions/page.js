...
const EmptyState = ({ filter }) => (
  <div className="py-20 text-center">
    ...
    <p className="mt-1 text-[#6b7280]">
      There are no {filter !== "all" ? statusMapping[filter] : ""} sessions to
      display.
    </p>
  </div>
);
...
const SessionRow = ({
  ...
  const timeRange = `${formatTime(session.startTime)} - ${formatTime(
    session.endTime
  )}`;

  // Determine if session is paid (static for now)
  const isPaid =
            ...
            <>
              ...
              <SessionButton
                session={{
                  ...
                  startTime: format(new Date(session.startTime), "h:mm a"),
                  endTime: format(new Date(session.endTime), "h:mm a"),
                  sessionType: "virtual",
                  status: "confirmed",
                }}
                userRole="tutor"
...
// Main component - Using JWT tokens instead of Firebase
export default function TutorSessions() {
    ...
    const fetchSessions = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${API_CONFIG.baseURL}/bookings/provider`,
          {
            headers: {
  ...
  const handleViewNotes = (sessionId) => {
    ...
    // Implement notes viewing logic here
  };
  ...
  const handleConfirmSession = async (sessionId) => {
    try {
      ...
      // Use API_CONFIG.baseURL instead of hardcoded URL
      const response = await fetch(
        `${API_CONFIG.baseURL}/bookings/${sessionId}/status`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleViewDetails = (sessionId) => {
    ...
    // Implement details viewing logic here
  };

  // Modified handler for messaging using JWT token
  const handleMessageStudent = async (sessionId, studentName) => {
    try {
      ...
      // Use apiClient to create or get conversation
      ...
      if (response.success) {
        ...
        router.push(`/tutor/messages?conversation=${conversation.id}`);
      } else {
